...
This repo includes a [Render Blueprint](https://render.com/docs/infrastructure-as-code) (`render.yaml`) that deploys the bot as a **Background Worker**, built from the existing `Dockerfile`. A Background Worker is used (rather than a Web Service) because the bot is a Telethon client that only ever makes outbound connections to Telegram — it never needs to accept inbound HTTP traffic.

Docker is the supported deploy path because several features (audio/video processing, OCR, PDF tooling, archive extraction, YouTube downloads) require system packages — `ffmpeg`, `aria2`, `tesseract-ocr`, `ghostscript`, `poppler-utils`, `p7zip`, the Deno runtime, etc. — that aren't available on Render's native Python runtime. The Blueprint also provisions an optional private service for [`bgutil-ytdlp-pot-provider`](https://github.com/Brainicism/bgutil-ytdlp-pot-provider) (mirroring the `bgutil-provider` service in `docker-compose.yml`) and a small persistent disk mounted at `state/` so permissions, plugin state, and the Telethon session survive restarts/redeploys.

### Steps

1. Push this repository to GitHub/GitLab.
2. In the [Render Dashboard](https://dashboard.render.com), click **New +** → **Blueprint**, and select your repo. Render detects `render.yaml` automatically.
3. During Blueprint setup, Render prompts you for the env vars marked `sync: false` (`BOT_TOKEN`, `API_ID`, `API_HASH`, `BOT_ADMINS`, and any optional integrations you want — see [Setup](#setup) above for where to get each one).
4. Click **Apply** to provision and deploy.

A plain `requirements.txt` and `Procfile` are also included for platforms/tooling that expect them, but they are **not** sufficient on their own for the full feature set on a non-Docker host — see the comment at the top of `requirements.txt` for the system packages you'd need to install yourself.

...
async def run_bot() -> None:
    ...
    # Run blocking
    async with bot:
        await run_until_disconnected_with_optional_health_server(bot)


async def run_until_disconnected_with_optional_health_server(bot: TelegramClient) -> None:
    """
    Run the bot until disconnected.

    If the PORT environment variable is set (Render sets this automatically for
    services deployed as a "Web Service"), also start a minimal HTTP server that
    responds 200 OK so Render's port detection succeeds. Background workers,
    local runs, and Docker Compose don't set PORT, so this is a no-op for them
    ...
    await runner.setup()
    site = web.TCPSite(runner, '0.0.0.0', int(port))  # noqa: S104
    await site.start()
